...
class CutPastePlus(CutPaste):
    ...
    def __init__(self, area_ratio=[0.02, 0.15], aspect_ratio=0.3, **kwags):
        super(CutPastePlus, self).__init__(**kwags)
        self.area_ratio = area_ratio
        self.aspect_ratio = aspect_ratio

    def __call__(self, img, paste_area):
        ...
        paste_location = paste_area[random.randint(0, len(paste_area) - 1)]  # middle point
        to_location_h = int(max(0, paste_location[0] - cut_h / 2))
        to_location_w = int(max(0, paste_location[1] - cut_w / 2))
...
class CutPastePlusScar(CutPaste):
    """Randomly copy one patche from the image and paste it somewere else.
    Args:
        width (list): width to sample from. List of [min, max]
        height (list): height to sample from. List of [min, max]
    ...
    def __init__(self, width=[10, 25], height=[10, 25], rotation=[-45, 45], **kwags):
        super(CutPastePlusScar, self).__init__(**kwags)
        self.width = width
        self.height = height
        self.rotation = rotation
    ...
    def __call__(self, img, paste_area):
        ...
        paste_location = paste_area[random.randint(0, len(paste_area) - 1)]  # middle point
        to_location_h = int(max(0, paste_location[0] - cut_h))
        to_location_w = int(max(0, paste_location[1] - cut_w))
...
class CutPastePlusUnion(object):
    def __init__(self, **kwags):
        self.normal = CutPastePlus(**kwags)
        self.scar = CutPastePlusScar(**kwags)

    def __call__(self, img, paste_area):
        r = random.uniform(0, 1)
        if r < 0.5:
            return self.normal(img, paste_area)
        else:
            return self.scar(img, paste_area)
