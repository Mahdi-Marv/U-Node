...
class IMAGENET30_TEST_DATASET(Dataset):
    def __init__(self, root_dir="/kaggle/input/imagenet30-dataset/one_class_test/one_class_test/", transform=None, count=-1):
        """
        Args:
        ...
        if count != -1:
            if count < len(self.img_path_list):
                self.img_path_list = self.img_path_list[:count]
                self.targets = self.targets[:count]
            else:
                t = len(self.img_path_list)
                for i in range(count - t):
                    self.img_path_list.append(random.choice(self.img_path_list[:t]))
                    self.targets.append(random.choice(self.targets[:t]))
